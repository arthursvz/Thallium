...
def main():
    ...
    if "-call" in args:
        ...
        args.remove("-call")
    if len(args) < 1:
        print("Usage: python3 bruteforce.py <file.enc> [min_len] [max_len] [charset] [output_file] [-call]")
        print("Default: min_len=1, max_len=4, charset=all printable ASCII (letters, digits, specials)")
        sys.exit(1)
    enc_path = args[0]
